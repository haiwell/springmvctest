...
function jqGridInclude()
{
    ...
    if (!_contextPath) {
        if (window.$GC) _contextPath = $GC.getContextPath();
        else _contextPath = "";
    }
    var pathtojsfiles = _contextPath + "/gcbase/js/jquery/jgrid/min/"; // need to be ajusted
    
    // set include to false if you do not want some modules to be included
    var modules = [
        { include: true, incfile:'i18n/grid.locale-zh.js'}, // jqGrid translation
        ...
        if(modules[i].include === true) {
        	...
        	filename = pathtojsfiles+modules[i].incfile;
       		if(jQuery.browser.safari ) {
				...
				//if (jQuery.browser.msie) {    // commented by richard
